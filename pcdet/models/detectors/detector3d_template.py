...
class Detector3DTemplate(nn.Module):
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            # print('cls_pred_size:', cls_preds.shape)
            # print('box_preds:', box_preds)
            # print('box_pred_size:', box_preds.shape)

            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                # print('done3')
                # print('num_class:', self.num_class)
                if not isinstance(cls_preds, list): # done
                    cls_preds = [cls_preds]
                    multihead_label_mapping = [torch.arange(1, self.num_class, device=cls_preds[0].device)]
                else:
                    multihead_label_mapping = batch_dict['multihead_label_mapping']

                print('multihead_label_mapping:', multihead_label_mapping)

                cur_start_idx = 0
                pred_scores, pred_labels, pred_boxes = [], [], []
                for cur_cls_preds, cur_label_mapping in zip(cls_preds, multihead_label_mapping):
                    # print('cur_cls_preds:', cur_cls_preds.shape)
                    # print('cur_label_mapping:', len(cur_label_mapping))
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False):
        ...
        # print("model_state_disk:", model_state_disk)
