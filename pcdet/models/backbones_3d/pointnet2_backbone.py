...
class PointNet2MSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        # print('num_point_features', self.num_point_features) # 128
        self.first_display1 = False
        self.first_display2 = False
...
class PointNet2MultiView(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        if not ('a2' in self.model_cfg['MV_CLASS']):
            ...
            # print('a2:', True)
        if mv_status == 1:
            points = [batch_dict['points'], batch_dict['points_a1'], batch_dict['points_a2']]
        elif mv_status == 2:
            points = [batch_dict['points'], batch_dict['points_a1']]
        elif mv_status == 3:
            points = [batch_dict['points'], batch_dict['points_a2']]
        point_features = []
        point_coords = []
        for k in range(len(points)):
            batch_idx, xyz, features = self.break_up_pc(points[k])
            ...
            features = l_features[0].permute(0, 2, 1).contiguous()  # (B, N, C)
            coords = torch.cat((batch_idx[:, None].float(), l_xyz[0].view(-1, 3)), dim=1)

            point_features.append(features.view(-1, features.shape[-1]))
            point_coords.append(coords)

        batch_dict['point_features'] = point_features[0]
        batch_dict['point_coords'] = point_coords[0]
        if mv_status == 1:
            batch_dict['point_a1_features'] = point_features[1]
            batch_dict['point_a1_coords'] = point_coords[1]
            batch_dict['point_a2_features'] = point_features[2]
            batch_dict['point_a2_coords'] = point_coords[2]
        elif mv_status == 2:
            batch_dict['point_a1_features'] = point_features[1]
            batch_dict['point_a1_coords'] = point_coords[1]
        elif mv_status == 3:
            batch_dict['point_a2_features'] = point_features[1]
            batch_dict['point_a2_coords'] = point_coords[1]
        # batch_dict['point_a1_ori_features'] = point_features[3]
        # batch_dict['point_a1_ori_coords'] = point_coords[3]
        # batch_dict['point_a2_ori_features'] = point_features[4]
        # batch_dict['point_a2_ori_coords'] = point_coords[4]

        # print('point_coords:', batch_dict['point_coords'])
        # print('gt_boxes:', batch_dict['gt_boxes'])
