...
def rotate_points_along_z(points, angle):
    ...
    # print('points_rot:', points_rot)
    return points_rot.numpy() if is_numpy else points_rot

def rotate_points_along_z_gpu(points, angle):
    """
    Args:
        points: (B, N, 3 + C)
        angle: (B), angle along z-axis, angle increases x ==> y
...
def mask_points_by_ground(points, limit_z):
    mask = (points[:, 3] >= limit_z[0]) & (points[:, 3] <= limit_z[1])
    return mask
