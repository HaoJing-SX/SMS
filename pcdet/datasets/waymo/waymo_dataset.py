...
class WaymoDataset(DatasetTemplate):
    ...
    def get_infos(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1):
        ...
        # print('process_single_sequence', process_single_sequence)
        # print('sample_sequence_file_list', sample_sequence_file_list)
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        # print('info', infos)
        # print('info_size:', len(infos)) # 1190 sum number
        # print('sampled_interval', sampled_interval) # 1

        point_offset_cnt = 0
        stacked_gt_points = []
        for k in range(0, len(infos), sampled_interval):
        # for k in range(0, 3, sampled_interval):
            ...
            # print('names %s' % k, names)
            # print('difficulty %s' % k, difficulty)
            # print('gt_boxes %s' % k, gt_boxes)
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or names[i] in used_classes:
                    with open(filepath, 'w') as f:
                        gt_points.tofile(f)
                        # print('done %s' % k)
                        # print('name:', names[i]) # Vehicle or Pedestrian or Cyclist
                        # print('used_classes:', used_classes) # ['Vehicle', 'Pedestrian', 'Cyclist']
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    waymo_infos_train = dataset.get_infos(
        raw_data_path=data_path / 'training_source',
        save_path=data_path / processed_data_tag, num_workers=workers, has_label=True,
        sampled_interval=1
    )
    ...
    waymo_infos_val = dataset.get_infos(
        raw_data_path=data_path / 'validation_source',
        save_path=data_path / processed_data_tag, num_workers=workers, has_label=True,
        sampled_interval=1
    )
