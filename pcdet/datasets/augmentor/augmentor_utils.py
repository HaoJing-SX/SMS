...
def global_rotation(gt_boxes, points, rot_range):
    ...
    # print('noise_rotation:', noise_rotation)
    # print('np.newaxis:', np.newaxis) # None
    # print('gt_boxes:', gt_boxes)
    return gt_boxes, points

def global_rotation2(gt_boxes, points, rot_angle):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    # a1
    points[0] = common_utils.rotate_points_along_z(points[0][np.newaxis, :, :], np.array([rot_angle[0]]))[0]
    gt_boxes[0][:, 0:3] = common_utils.rotate_points_along_z(gt_boxes[0][np.newaxis, :, 0:3], np.array([rot_angle[0]]))[0]
    gt_boxes[0][:, 6] += rot_angle[0]
    # a2
    points[1] = common_utils.rotate_points_along_z(points[1][np.newaxis, :, :], np.array([rot_angle[1]]))[0]
    gt_boxes[1][:, 0:3] = common_utils.rotate_points_along_z(gt_boxes[1][np.newaxis, :, 0:3], np.array([rot_angle[1]]))[0]
    gt_boxes[1][:, 6] += rot_angle[1]

    return gt_boxes, points

def point_rotation(points, rot_range):
    """
    Args:
        points: (M, 3 + C),
        rot_range: angle
    ...
    # print('gt_boxes1:', gt_boxes)

    points = common_utils.rotate_points_along_z(points[np.newaxis, :, :], np.array([rot_angle]))[0]
