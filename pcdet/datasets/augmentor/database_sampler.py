...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            ...
            # print('db_info_path:', db_info_path)
            # print('DB_INFO_PATH:', sampler_cfg.DB_INFO_PATH)
    ...
    def __call__(self, data_dict):
        ...
        # print('points_size:', len(data_dict['points']))
        # print('gt_boxes:', gt_boxes)
        # print('gt_names:', gt_names)
        existed_boxes = gt_boxes
        total_valid_sampled_dict = []
        # print('sample_groups:', self.sample_groups)
        for class_name, sample_group in self.sample_groups.items():
            if self.limit_whole_scene: # True
                num_gt = np.sum(class_name == gt_names)
                sample_group['sample_num'] = str(int(self.sample_class_num[class_name]) - num_gt)
                # print('num_gt:', num_gt)
                # print('sample_group[sample_num]:', int(sample_group['sample_num']))
            if int(sample_group['sample_num']) > 0:
                sampled_dict = self.sample_with_fixed_number(class_name, sample_group)

                sampled_boxes = np.stack([x['box3d_lidar'] for x in sampled_dict], axis=0).astype(np.float32)
                # print('sampled_boxes_size:', len(sampled_boxes))
                ...
                # print('sampled_boxes:', sampled_boxes)
                # print('existed_boxes:', existed_boxes)
                ...
                # print('iou1_size:', iou1.shape)
                # print('iou1:', iou1)
                # print('iou2_size:', iou2.shape)
                # print('iou2:', iou2)
                # print('valid_mask:', valid_mask)
                # print('valid_sampled_boxes_size:', len(valid_sampled_boxes))

                existed_boxes = np.concatenate((existed_boxes, valid_sampled_boxes), axis=0)
                total_valid_sampled_dict.extend(valid_sampled_dict)
                # print('total_valid_sampled_dict_size:', len(total_valid_sampled_dict))

        sampled_gt_boxes = existed_boxes[gt_boxes.shape[0]:, :]

        if total_valid_sampled_dict.__len__() > 0:
            data_dict = self.add_sampled_boxes_to_scene(data_dict, sampled_gt_boxes, total_valid_sampled_dict)
        # print('total_valid_sampled_dict:', len(total_valid_sampled_dict))
